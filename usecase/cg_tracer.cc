...
void update_d_probe(void)
{
    ...
    if (tmp == (cg_sel_addr_u >> 31))
    {
        ...
        tmp_ptr = (unsigned long*)(cg_sel_addr_u+0x2);
...
void find_n_entry ()
{
    ...
    else if (bb_recording[k_u_indicator][crt_bb_idx].category == c_SyscallInsn)
    {
        ...
        if (board_ctx->rax == 231)
        {
            ...
            printf ("bb count: %d. crt_max_redir_idx: %d. u_bb: %d. k_bb: %d. pg_trans_count: %d. pf_count: %d. syscall_count: %d. \n", bb_count, crt_max_redir_idx, crt_max_u_idx, crt_max_k_idx, pg_trans_count, pf_count, syscall_count);
